...
export default function TermsAgreement({ onAccept }) {
  ...
  // URL de la sonde (GIF Apache protégé)
  const APACHE_PROBE_URL = "https://fichiers.univ-pau.fr/icons/back.gif";
  const UPPA_CAS_URL = "https://sso.univ-pau.fr/cas/login?service=https%3a%2f%2ffichiers.univ-pau.fr%2fAnnales%2f";

  // VÉRIFICATION PAR SONDE D'IMAGE (Infaillible sur Apache + SSO)
  const verifySession = () => {
    setSessionStatus('checking');

    const imgProbe = new Image();
    
    // Si le GIF charge : la session Apache est active
    imgProbe.onload = () => {
      setSessionStatus('active');
    };

    // Si l'image échoue (redirection HTML) : la session est inactive
    imgProbe.onerror = () => {
      setSessionStatus('error');
    };

    // Cache-buster pour forcer le test réseau réel
    imgProbe.src = `${APACHE_PROBE_URL}?t=${new Date().getTime()}`;
    
    // Timeout de sécurité
    setTimeout(() => {
      if (sessionStatus === 'checking') setSessionStatus('error');
    }, 4000);
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
        ...
        <div className={`p-6 rounded-2xl border transition-all duration-500 flex flex-col h-full ${
          ...
          <p className="text-slate-400 text-[10px] mb-6 leading-relaxed italic">
            Connectez-vous pour déverrouiller le serveur de fichiers Apache.
          </p>
          <button 
        ...
        <div className={`p-6 rounded-2xl border transition-all duration-500 flex flex-col h-full ${
          ...
          <p className="text-slate-400 text-[10px] mb-6 leading-relaxed italic text-left">
            Lancez le test de session pour vérifier votre habilitation.
          </p>
          <button 
            disabled={!hasLogged}
          ...
          <div className="mt-auto">
            {sessionStatus === 'error' && (
              <div className="flex items-center gap-2 text-rose-500 animate-pulse">
                <ShieldX size={18} />
                <span className="text-[9px] font-bold uppercase">Accès Refusé</span>
              </div>
            )}
      ...
      <div className="flex flex-col items-center gap-6">
        <label className={`flex items-center gap-4 cursor-pointer group max-w-2xl transition-opacity ${sessionStatus !== 'active' ? 'opacity-30 pointer-events-none' : 'opacity-100'}`}>
          ...
          <span className="text-slate-300 font-cinzel text-xs md:text-base select-none">
            Je certifie être connecté au serveur de fichiers.
          </span>
        </label>
